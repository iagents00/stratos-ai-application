...
Antes de producción: aplicar y revisar la migración **245_rails_circuito_confirmado.sql** en staging con los triggers y políticas reales; validar roles y una gestión completa con recarga; coordinar actualización de frontend y de clientes abiertos. La RPC antigua pasa a rechazar gestiones sin el nuevo contrato, por lo que un frontend antiguo no debe seguir operando Rails durante la transición. Publicar ambos cambios de manera coordinada, ejecutar el centinela de RPC y habilitar primero un piloto. Conservar las evidencias escritas ante cualquier reversión de interfaz.
