...
const interpreter = nodes.filter(n => n.name === 'Parse Intérprete');
const pick = nodes.filter(n => n.name === 'Parse Pick');
if (interpreter.length !== 1 || pick.length !== 1) throw new Error('Expected one interpreter and one picker node.');
interpreter[0].parameters.jsCode = patchBulkInterpreter(interpreter[0].parameters.jsCode);
pick[0].parameters.jsCode = patchBulkPick(pick[0].parameters.jsCode);
