...
let nodes = structuredClone(before.nodes);
const parseNodes = nodes.filter(node => node.name === 'Parse Pick');
if (parseNodes.length !== 1) throw new Error('Expected exactly one Parse Pick node.');
parseNodes[0].parameters.jsCode = patchTeamPendingGuard(parseNodes[0].parameters.jsCode);
nodes = patchTeamPendingDispatch(nodes);
