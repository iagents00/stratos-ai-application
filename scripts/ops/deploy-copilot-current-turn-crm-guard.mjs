...
import { patchCurrentTurnCrmInterpreter } from './copilot-current-turn-crm-guard.mjs';
...
if (interpreter.length !== 1) throw new Error('Expected one Parse Intérprete node.');
interpreter[0].parameters.jsCode = patchCurrentTurnCrmInterpreter(interpreter[0].parameters.jsCode);
...
if (!relToRepository.startsWith('..') || relToRepository === '') {
  throw new Error('Workflow exports can contain credentials; backup outside the repository.');
}
...
if (current.versionId !== before.versionId || current.activeVersionId !== before.activeVersionId) {
  throw new Error('Concurrent workflow update; stopped.');
}
