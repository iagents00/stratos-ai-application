...
import { patchZoomConsistency } from './copilot-zoom-consistency.mjs';
...
const patched = patchZoomConsistency(before.nodes, before.connections);
const changed = JSON.stringify(patched.nodes) !== JSON.stringify(before.nodes) || JSON.stringify(patched.connections) !== JSON.stringify(before.connections);
...
const updated = await api(`/workflows/${id}`, 'PUT', { name: before.name, nodes: patched.nodes, connections: patched.connections, settings: before.settings });
...
if (JSON.stringify(after.connections) !== JSON.stringify(patched.connections)) throw new Error('Connections differ from scoped patch.');
if (JSON.stringify(after.nodes) !== JSON.stringify(patched.nodes)) throw new Error('Saved nodes differ from scoped patch.');
