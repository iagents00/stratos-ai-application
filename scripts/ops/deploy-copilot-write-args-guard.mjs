...
async function api(path, method = 'GET', body) {
  const response = await fetch(base + path, { method, signal: AbortSignal.timeout(30000), headers: { 'X-N8N-API-KEY': process.env.N8N_API_KEY, 'Content-Type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body) });
...
const matches = nodes.filter(node => node.name === 'Parse Pick');
if (matches.length !== 1) throw new Error('Expected exactly one Parse Pick node.');
matches[0].parameters.jsCode = patchWriteArgsGuard(matches[0].parameters.jsCode);
...
if (after.activeVersionId !== updated.versionId) { await api(`/workflows/${id}/activate`, 'POST', { versionId: updated.versionId }); after = await api(`/workflows/${id}`); }
