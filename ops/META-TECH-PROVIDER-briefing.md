...
Los pasos 3-6 y 10 de la ruta ya no dependen de nadie: el código está escrito,
compila y está apagado por defecto.

| Pieza | Archivo |
|---|---|
| Librería de Embedded Signup | `src/lib/whatsapp-signup.js` |
| Vista "Conectar WhatsApp" | `src/app/views/ConectarWhatsApp.jsx` |
| Feature flag + config de Meta | `src/clients/_shared/defaults.js` |
| Entrada de navegación | `src/app/constants/navigation.js` |
| Render de la vista | `src/app/App.jsx` |

### Cómo se prende el día que Meta apruebe

En `src/clients/<cliente>/config.js`:
...
Nada más. Con el flag apagado el módulo no existe para nadie — verificado.
...
- **El módulo es visible para clientes externos y para asesores.** Conectar su
  propio WhatsApp es justo lo que tienen que poder hacer solos — es el punto
  entero de ser Tech Provider.
...
| 10. Embedded Signup propio | ✅ **Hecho** (apagado por flag) |
