-- Fix: en `entregas` y `cambios` el LIMIT recortaba ANTES de ordenar, así que con
-- muchos registros el informe podía quedarse con los más viejos en vez de los más
-- recientes. Ahora se ordena dentro de la subconsulta y recién ahí se recorta.
-- (Se reemplaza la función completa: en plpgsql no hay parches parciales.)
...
begin
  ...
  select coalesce(jsonb_agg(x order by cuando desc), '[]'::jsonb) into v_entregas
  from (
    select t.updated_at as cuando,
           jsonb_build_object(
             'titulo',  t.titulo,
             'proyecto', pr.nombre,
             'quien',   pf.name,
  ...
  select coalesce(jsonb_agg(x order by cuando desc), '[]'::jsonb) into v_bitacora
  from (
    select u.created_at as cuando,
           jsonb_build_object(
             'cliente', l.name,
             'tipo',    u.tipo,
             'texto',   u.texto,
  ...
  if v_cerebro then
    ...
    select coalesce(jsonb_agg(x order by dia desc), '[]'::jsonb) into v_reuniones
    from (
      select substring(d.path from '(20[0-9]{2}-[0-9]{2}-[0-9]{2})')::date as dia,
             jsonb_build_object(
    ...
    select coalesce(jsonb_agg(x order by dia desc), '[]'::jsonb) into v_cambios
    from (
      select substring(b.bloque from '^- (20[0-9]{2}-[0-9]{2}-[0-9]{2})')::date as dia,
             jsonb_build_object(
