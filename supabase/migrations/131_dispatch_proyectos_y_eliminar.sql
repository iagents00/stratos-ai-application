-- Amplía mkt_nlu_dispatch: agrega proyectos + eliminar/restaurar.
-- ADITIVO: los casos existentes quedan idénticos (Duke/marketing no cambia).
...
begin
  ...
  case coalesce(p_tool_name,'')
    ...
    -- ── NUEVO (25-jul): proyectos + eliminar/restaurar (borrado SUAVE, reversible) ──
    ...
    when 'restore_task' then
      ...
      v_reply := 'Puedo ayudarte con: tu día, crear tareas, avisar que empezaste ("ya empecé…"), completarlas ("ya terminé…"), posponerlas ("pospón…"), eliminarlas ("elimina la tarea…"), crear/listar/archivar proyectos, mover propiedades del pipeline, solicitudes de diseño (y asignarlas), resumen del pipeline, pendientes de una persona y buscar el drive de una propiedad.';
  end case;
