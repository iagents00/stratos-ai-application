-- Replace Ivan's NSG capture number; retain the former channel and all lead history.
-- Infobip sender: 5216861469109. Existing authenticated webhook stays stable.
-- Rollback: restore the previous RPC definition and channel active flags from backup,
-- and restore the Infobip subscription resource filter. Never delete the WABA.
DO $guard$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.profiles
    WHERE organization_id='4a17b181-35d2-41b3-b639-6e0bd4c38acc'
      AND id='54ef3b9e-44f3-46d5-84c0-b8acab027adf' AND active) THEN
    RAISE EXCEPTION 'Active NSG Ivan profile required';
  END IF;
  IF EXISTS (SELECT 1 FROM public.whatsapp_numero_asesor
    WHERE organization_id='4a17b181-35d2-41b3-b639-6e0bd4c38acc'
      AND public.fn_phone_canon(numero_whatsapp) IN ('526861469109','526861936914')
      AND (asesor_id IS DISTINCT FROM '54ef3b9e-44f3-46d5-84c0-b8acab027adf'::uuid
        OR proveedor IS DISTINCT FROM 'infobip')) THEN
    RAISE EXCEPTION 'NSG number belongs to a different advisor/provider';
  END IF;
END;
$guard$;
...
SET active=false, updated_at=now(),
    nota='Retirado de captura CRM NSG el 2026-09-09; reemplazado por +526861469109. Historial conservado.'
WHERE organization_id='4a17b181-35d2-41b3-b639-6e0bd4c38acc'
  AND public.fn_phone_canon(numero_whatsapp)='526861936914'
  AND asesor_id='54ef3b9e-44f3-46d5-84c0-b8acab027adf' AND proveedor='infobip';

INSERT INTO public.whatsapp_numero_asesor
  (organization_id,numero_whatsapp,asesor_id,asesor_name,proveedor,active,nota,platform_type,estado_conexion,verified_name)
SELECT p.organization_id,'+526861469109',p.id,p.name,'infobip',true,
  'NSG inbound; Infobip sender 5216861469109; reemplaza 6914 el 2026-09-09',
  'CLOUD_API','desconocido','Iván Rodríguez NSG - Stratos AI'
...
WHERE p.organization_id='4a17b181-35d2-41b3-b639-6e0bd4c38acc'
  ...
  AND NOT EXISTS (SELECT 1 FROM public.whatsapp_numero_asesor c
    WHERE c.organization_id=p.organization_id
      AND public.fn_phone_canon(c.numero_whatsapp)='526861469109');

UPDATE public.whatsapp_numero_asesor SET active=true,updated_at=now()
WHERE organization_id='4a17b181-35d2-41b3-b639-6e0bd4c38acc'
...
DECLARE
  ...
  v_recipient constant text := '526861469109';
