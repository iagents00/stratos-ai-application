 ...
 *
 * Ahora las filas salen de `profiles` (vía useTeam) unidas con quien tenga leads,
 * y cada número se calcula de leadsData con los mismos criterios que el
...
const Team = ({ T: _T, leadsData = [] }) => {
  ...
  // La unión importa: en Duke hay leads a nombre de super_admins, y si filtrara
  // solo por rol `asesor` esas personas desaparecerían de la tabla.
  const rows = useMemo(() => {
    ...
    return [...porNombre.values()].map(p => {
      ...
      return {
        ...
        score: suyos.length ? Math.round(suyos.reduce((s, l) => s + (l.sc || 0), 0) / suyos.length) : 0,
      };
  ...
  const COLS = "minmax(180px, 1.6fr) 70px 70px 70px 100px 110px";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <G np T={T}>
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <p style={{ fontSize: 13, fontWeight: 500, color: T.txt, fontFamily: font, margin: 0 }}>Rendimiento del Equipo</p>
          ...
          <div key={m.name} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: 13, fontWeight: 600, color: T.txt, fontFamily: font, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.name}</p>
                <p style={{ fontSize: 10, color: T.txt3, fontFamily: font, marginTop: 2 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 11, color: m.conv >= 15 ? T.emerald : m.conv >= 5 ? T.blue : T.txt3, fontWeight: 600, fontFamily: fontDisp }}>{m.conv}%</span>
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <G T={T}>
          <p style={{ fontSize: 13, fontWeight: 700, color: T.txt, marginBottom: 12 }}>Leads por asesor</p>
