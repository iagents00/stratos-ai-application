...
import IACRMPlanes from "./IACRMPlanes.jsx";
...
const ChannelBadge = ({ ch, T, isLight }) => {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 10.5, fontWeight: 500, color: c, background: `${c}12`, border: `1px solid ${c}28`, padding: "2px 7px", borderRadius: 6, fontFamily: fontDisp, letterSpacing: "0.04em" }}>
...
const StatusChip = ({ s }) => {
  ...
  return <span style={{ fontSize: 10.5, fontWeight: 500, color: c, fontFamily: fontDisp }}>{l}</span>;
};
...
const AgentToggle = ({ active, onChange, T, isLight }) => (
  <button onClick={onChange} style={{
    ...
    <span style={{ fontSize: 11, fontWeight: 500, color: active ? T.accent : T.txt3, fontFamily: fontDisp }}>{active ? "Activo" : "Pausado"}</span>
  </button>
);
...
const IACRM = ({ oc, T: _T, theme = "dark" }) => {
  ...
  //
  // Mismo criterio que el módulo "Planes" del menú, que ya se esconde en nativo
  // (src/app/constants/navigation.js). Las otras dos pestañas no tienen nada de
  // compra, así que el módulo sigue sirviendo igual en el teléfono.
  //
  // Los planes se contratan por la web.
  const TABS = [
    { id: "agentes", label: "Mis Agentes" },
    { id: "canales", label: "Canales" },
    ...(isNativeApp() ? [] : [{ id: "planes", label: "Planes" }]),
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[{ c: WA, l: "WhatsApp" }, { c: TG, l: "Telegram" }].map(({ c, l }) => (
              <div key={l} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 11px", borderRadius: 9, background: `${c}10`, border: `1px solid ${c}28` }}>
                ...
                <span style={{ fontSize: 11.5, fontWeight: 500, color: c, fontFamily: fontDisp }}>{l}</span>
              </div>
            ))}
          </div>
          {/* CTA */}
          ...
          <G T={T}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                {[{ c: WA, l: "WhatsApp" }, { c: TG, l: "Telegram" }].map(({ c, l }) => (
                  <span key={l} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: T.txt3, fontFamily: font }}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                      ...
                      {!isActive && (
                        <span style={{ fontSize: 10, fontWeight: 500, color: T.txt3, background: isLight ? "rgba(15,23,42,0.06)" : "rgba(255,255,255,0.06)", padding: "2px 6px", borderRadius: 5, letterSpacing: "0.07em", fontFamily: fontDisp }}>PAUSADO</span>
                      )}
                    </div>
                  ...
                  <div style={{ display: "flex", gap: 16, alignItems: "center", flexShrink: 0 }}>
                    {[
                      ...
                      <div key={m.l} style={{ textAlign: "center" }}>
                        ...
                        <p style={{ margin: 0, fontSize: 10, color: T.txt3, fontFamily: font, textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 2 }}>{m.l}</p>
                      </div>
                    ))}
                  </div>
                  )}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-around", padding: "0 18px 13px", gap: 8 }}>
                    {[
                      ...
                      <div key={m.l} style={{ textAlign: "center", flex: 1 }}>
                        <p style={{ margin: 0, fontSize: 17, fontWeight: 500, color: m.c2, fontFamily: fontDisp, lineHeight: 1, letterSpacing: "-0.03em" }}>{m.v}</p>
                        <p style={{ margin: 0, fontSize: 10.5, color: T.txt3, fontFamily: font, textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 3 }}>{m.l}</p>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <div style={{ borderTop: `1px solid ${c}${isLight ? "22" : "16"}` }}>
                    ...
                    <div style={{ padding: "14px 18px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      <div style={{ padding: "11px 14px", borderRadius: 11, background: isLight ? `${c}08` : `${c}06`, border: `1px solid ${c}${isLight ? "20" : "14"}` }}>
                        <p style={{ margin: "0 0 5px", fontSize: 10.5, fontWeight: 500, color: cSafe, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: fontDisp }}>Descripción</p>
                        <p style={{ margin: 0, fontSize: 12, color: T.txt2, fontFamily: font, lineHeight: 1.6 }}>{agent.description}</p>
                      </div>
                      <div style={{ padding: "11px 14px", borderRadius: 11, background: isLight ? "rgba(15,23,42,0.03)" : "rgba(255,255,255,0.025)", border: `1px solid ${T.border}` }}>
                        <p style={{ margin: "0 0 7px", fontSize: 10.5, fontWeight: 500, color: T.txt3, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: fontDisp }}>Se activa cuando</p>
                        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
                          <Zap size={12} color={cSafe} strokeWidth={2.5} />
                          <p style={{ margin: 0, fontSize: 12, color: T.txt, fontFamily: fontDisp, fontWeight: 400 }}>{agent.trigger}</p>
                        </div>
                        <p style={{ margin: "0 0 5px", fontSize: 10.5, fontWeight: 500, color: T.txt3, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: fontDisp }}>Notifica al asesor</p>
                        <p style={{ margin: 0, fontSize: 11.5, color: isLight ? "#0066BB" : TG, fontFamily: font, lineHeight: 1.5 }}>{agent.teamAlert}</p>
                      </div>
                    </div>

                    {/* Conversaciones */}
                    <div style={{ padding: "0 18px 14px" }}>
                      <p style={{ margin: "0 0 10px", fontSize: 10.5, fontWeight: 500, color: T.txt3, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: fontDisp }}>Últimas conversaciones</p>
                          ...
                          <div key={i} style={{ padding: "11px 13px", borderRadius: 12, background: isLight ? "#F7F9FC" : "rgba(255,255,255,0.025)", border: `1px solid ${T.border}`, boxShadow: isLight ? T.shadow1 : "none" }}>
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 7 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                <div>
                                  <p style={{ margin: 0, fontSize: 12.5, fontWeight: 500, color: T.txt, fontFamily: fontDisp }}>{a.name}</p>
                                  <ChannelBadge ch={a.channel} />
                                </div>
                              </div>
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ fontSize: 10.5, color: T.txt3, fontFamily: font }}>{a.time}</span>
                            ...
                            <div style={{ marginLeft: 36, padding: "8px 12px", borderRadius: "4px 12px 12px 12px", background: isLight ? `${c}08` : `${c}10`, border: `1px solid ${c}${isLight ? "18" : "1C"}` }}>
                              <p style={{ margin: 0, fontSize: 12, color: T.txt2, fontFamily: font, lineHeight: 1.58, whiteSpace: "pre-line" }}>{a.msg}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ padding: "0 18px 16px", display: "flex", gap: 8 }}>
                      <button
                        ...
                        style={{
                          ...
                          color: cSafe, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: fontDisp,
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: `1px solid ${T.border}`,
                          color: T.txt3, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontFamily: fontDisp, transition: "all 0.15s",
                        }}
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 16 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                        ...
                        <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10.5, fontWeight: 500, color: ch.color, background: `${ch.color}12`, border: `1px solid ${ch.color}28`, padding: "2px 7px", borderRadius: 6, fontFamily: fontDisp }}>
                          <span style={{ width: 4, height: 4, borderRadius: "50%", background: ch.color, animation: "pulse 2s ease-in-out infinite" }} />
                          {ch.status.toUpperCase()}
                        </span>
                      </div>
                      <p style={{ margin: 0, fontSize: 12, color: T.txt3, fontFamily: font }}>{ch.number}</p>
                    </div>
                  </div>
                  <p style={{ margin: "0 0 16px", fontSize: 12, color: T.txt2, fontFamily: font, lineHeight: 1.6 }}>{ch.desc}</p>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                    {[
                      ...
                      <div key={l} style={{ padding: "9px 12px", borderRadius: 10, background: isLight ? "rgba(15,23,42,0.03)" : "rgba(255,255,255,0.03)", border: `1px solid ${T.border}`, textAlign: "center" }}>
                        ...
                        <p style={{ margin: 0, fontSize: 10.5, color: T.txt3, fontFamily: font, marginTop: 2 }}>{l}</p>
                      </div>
                    ))}
                  </div>
                  <button
                    onClick={() => oc(`Configurar canal ${ch.name}`)}
                    style={{ width: "100%", padding: "9px", borderRadius: 10, background: `${ch.color}0C`, border: `1px solid ${ch.color}28`, color: ch.color, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: fontDisp, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.18s" }}
                ...
                return (
                  <div key={s.step} style={{ display: "flex", gap: 14, paddingBottom: i < 5 ? 16 : 0, borderBottom: i < 5 ? `1px solid ${T.border}` : "none", marginBottom: i < 5 ? 16 : 0 }}>
                    ...
                    <div style={{ flex: 1, paddingTop: 4, minWidth: 0 }}>
                      ...
                      <p style={{ margin: 0, fontSize: 12, color: T.txt3, fontFamily: font, lineHeight: 1.55 }}>{s.desc}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {/* ══════════════════════════════════════════════
          TAB: PLANES
      ══════════════════════════════════════════════ */}
      {tab === "planes" && <IACRMPlanes T={T} isLight={isLight} oc={oc} />}
