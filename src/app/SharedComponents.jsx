...
export const Pill = ({ children, color = P.accent, s, isLight = false }) => {
  ...
  return (
    <span style={{
      ...
      fontSize: s ? 11 : 12, fontWeight: 500, color: textColor,
      background: bgGrad,
        ...
        <div style={{
          ...
          {!adding && (
            <>
                ...
                <div style={{ padding: "9px 10px 8px", borderBottom: `1px solid ${P.border}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 9px", borderRadius: 8, background: P.glass, border: `1px solid ${P.border}` }}>
                    ...
                    <input
                      ...
                      style={{ flex: 1, border: "none", background: "transparent", outline: "none", color: P.txt, fontSize: 12.5, fontFamily: font, minWidth: 0 }}
                    />
                  </div>
                </div>
              )}
              <div style={{ maxHeight: 220, overflowY: "auto" }}>
                {filtered.length === 0 && (
                  <div style={{ padding: "16px 14px", textAlign: "center", fontSize: 12, color: P.txt3, fontFamily: font }}>
                ...
                {filtered.map(opt => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ width: 22, height: 22, borderRadius: "50%", background: `${color}18`, border: `1px solid ${color}38`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 11, fontWeight: 500, color, fontFamily: fontDisp }}>
              ...
              <button type="button" onClick={() => { setAdding(true); setNewVal(query); }} style={{
                ...
                color, fontSize: 12.5, fontWeight: 500, fontFamily: fontDisp, letterSpacing: "0.01em",
                textAlign: "left",
              }}
          ...
          {adding && (
            <div style={{ padding: 10 }}>
              <p style={{ fontSize: 10.5, fontWeight: 500, color, letterSpacing: "0.08em", textTransform: "uppercase", fontFamily: fontDisp, margin: "0 0 7px 2px" }}>{newLabel}</p>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                />
                <button type="button" onClick={() => { setAdding(false); setNewVal(""); }} style={{ height: 34, padding: "0 10px", borderRadius: 8, background: "transparent", border: `1px solid ${P.border}`, color: P.txt3, fontSize: 12, fontWeight: 400, cursor: "pointer", fontFamily: font }}>
                  Cancelar
                </button>
                <button type="button" disabled={!newVal.trim()} onClick={handleAdd} style={{ height: 34, padding: "0 12px", borderRadius: 8, background: newVal.trim() ? color : "rgba(255,255,255,0.04)", border: `1px solid ${newVal.trim() ? color : P.border}`, color: newVal.trim() ? "#041016" : P.txt3, fontSize: 12, fontWeight: 500, cursor: newVal.trim() ? "pointer" : "not-allowed", fontFamily: fontDisp, letterSpacing: "0.01em" }}>
                  Guardar
                </button>
              </div>
              <p style={{ fontSize: 10.5, color: P.txt3, margin: "7px 2px 0", fontFamily: font }}>Disponible al registrar otros clientes más adelante.</p>
            </div>
          )}
        </div>
      )}
