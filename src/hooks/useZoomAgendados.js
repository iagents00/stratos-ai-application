 ...
 * public.zoom_agendados (migración 027) para la organización del usuario activo.
 * RLS aísla por organización, así que todas las queries van con el anon key del
 * usuario logueado — no se filtra org a mano (igual que useScheduledCalls).
 *
 * Devuelve:
 ...
 *                 migración 027 aún no se aplicó en este proyecto Supabase.
...
// no existe todavía → la migración 027 no se ha aplicado en este proyecto.
...
export function useZoomAgendados() {
  ...
  const createRow = useCallback(async (payload) => {
    ...
    const { error: err } = await supabase
      .from(TABLE)
      .insert([{ ...sanitize(payload), organization_id: orgId }]);
  ...
  return { rows, loading, error, refetch, createRow, updateRow, removeRow };
}
