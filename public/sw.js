...
const CACHE_VERSION = 'stratos-v160'; // v160: fix Safari — el átomo del sidebar (giro + brillo), el dot "Live" y el pill "Centro de Inteligencia" (anillo pulsante + beam) se congelaban SOLO en Safari por el bug de WebKit que detiene animaciones dentro de un ancestro con backdrop-filter (sidebar/header). Fix: will-change:transform en los elementos animados para promoverlos a su propia capa GPU. Otros navegadores intactos. // v158: MetaPanel (Plan Estratégico) — pantalla completa como una vista del CRM + rediseño ERP×Apple: barra superior sticky translúcida, edición discreta en hover/focus, filas con hover elevado; Lista de Acción a ancho completo con tipografía más grande, responsable a la izquierda y prioridad/fecha en columnas alineadas (borrar flotante), y layout móvil dedicado con el check a la derecha (sin gutter izquierdo). (v157: fix "Importing a module script failed" — recuperación escalonada de chunk viejo tras deploy, se conserva.)
