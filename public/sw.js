...
const CACHE_VERSION = 'stratos-v161'; // v161: fix Safari REAL — el átomo, el dot "Live" y el anillo del "Centro de Inteligencia" (todas animaciones ...infinite) se congelaban porque `html[data-hidden]` quedaba PEGADO en Safari (no dispara visibilitychange al volver a la ventana / restaura de bfcache con pageshow) y mobile-perf.css pausa las animaciones infinitas con ese flag. Fix: index.html re-sincroniza data-hidden también en pageshow y focus. (v160 probó will-change, que NO era: la animación estaba paused por CSS, no por capa GPU — se deja igual, es inocuo.) // v158: MetaPanel (Plan Estratégico) — pantalla completa como una vista del CRM + rediseño ERP×Apple: barra superior sticky translúcida, edición discreta en hover/focus, filas con hover elevado; Lista de Acción a ancho completo con tipografía más grande, responsable a la izquierda y prioridad/fecha en columnas alineadas (borrar flotante), y layout móvil dedicado con el check a la derecha (sin gutter izquierdo). (v157: fix "Importing a module script failed" — recuperación escalonada de chunk viejo tras deploy, se conserva.)
