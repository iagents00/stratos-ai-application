...
const CACHE_VERSION = 'stratos-v160'; // v160: la barra inferior y la hoja "Menú" de la app movil adoptan el estilo del sidebar de PC: pastilla activa elevada blanco-menta (chip radio 14 + borde + sombra), icono/label activos en #E9FCF4 (no verde). Solo estilos moviles. // v159: "Usuarios" sale de la grilla de apps -> seccion "Configuracion" (grilla queda en 12). Escritorio: footer Configuracion con boton Usuarios (+ engranaje del sidebar). Movil: "Gestion de usuarios" en Configuracion. Solo admins. // v158: MetaPanel (Plan Estratégico) — pantalla completa como una vista del CRM + rediseño ERP×Apple: barra superior sticky translúcida, edición discreta en hover/focus, filas con hover elevado; Lista de Acción a ancho completo con tipografía más grande, responsable a la izquierda y prioridad/fecha en columnas alineadas (borrar flotante), y layout móvil dedicado con el check a la derecha (sin gutter izquierdo). (v157: fix "Importing a module script failed" — recuperación escalonada de chunk viejo tras deploy, se conserva.)
